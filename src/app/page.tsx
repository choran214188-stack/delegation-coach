    ...
    <main className="mx-auto max-w-2xl px-4 py-8 sm:py-12">
      <header className="mb-8">
        ...
        <p className="mt-2 text-sm leading-relaxed text-slate-600">
          위임하려는 <b>업무</b>와 그 일을 맡길 <b>팀원의 상황·성향</b>을 입력하면, 그 팀원이 이 업무에
          대해 어느 발달수준(D1~D4)에 있는지 진단하고 지금 어떻게 지원해야 할지 알려드립니다.
        </p>
      </header>
      ...
      {status !== "done" && (
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Field label="대상 팀원 이름 · 호칭" hint="선택 사항">
            <input
              value={memberName}
              onChange={(e) => setMemberName(e.target.value)}
              placeholder="예: 김대리, 박팀원"
          ...
          <Field
            label="이 팀원의 경험 · 역량 · 성향 · 현재 상황"
            required
            hint="이 업무 기준으로 얼마나 해봤는지, 잘하는지, 요즘 의욕은 어떤지 자유롭게 적어주세요."
          >
            <textarea
              value={memberContext}
              onChange={(e) => setMemberContext(e.target.value)}
              rows={5}
              placeholder="예: 입사 6개월 차. 성실하고 배우려는 의욕은 높은데 이런 규모의 행사는 처음이라 어디서부터 해야 할지 막막해 함. 실수할까 봐 자주 물어봄."
              className="w-full resize-none rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200"
            />
          </Field>
